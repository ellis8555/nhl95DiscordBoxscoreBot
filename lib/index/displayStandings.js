...
export default async function displayStandings(seasonGamesChannelId, { client, standings, messageContent, coaches }) {
    ...
    for (const standing of sortedStandings) {
        ...
        const winsXPos = messageContent === 'BUBBLE' ? 250 : 370
        // statsX begins at xPos 370 which is wins
        // single character is 28 width
        if(messageContent === 'BUBBLE'){
            // wins
            ctx.fillText(wins, winsXPos, yPos + teamLogoHeight / 4);
            // 1rst seperator
            ctx.fillText(seperator, winsXPos + 42, yPos + teamLogoHeight / 4);
            // losses
            ctx.fillText(losses, winsXPos + 42 + 28, yPos + teamLogoHeight / 4);
            // 2nd seperator
            ctx.fillText(seperator, winsXPos + 42 + 28 + 28 + 14, yPos + teamLogoHeight / 4);
            // ties
            ctx.fillText(ties, winsXPos + 42 + 28 + 28 + 14 + 28, yPos + teamLogoHeight / 4);
            // 3rd seperator
            ctx.fillText(seperator, winsXPos + 42 + 28 + 28 + 14 + 28 + 42, yPos + teamLogoHeight / 4);
            // otl
            ctx.fillText(otl, winsXPos + 42 + 28 + 28 + 14 + 28 + 42 + 42, yPos + teamLogoHeight / 4);        
        } else {
            // wins
            ctx.fillText(wins, winsXPos, yPos + teamLogoHeight / 4);
            // 1rst seperator
            ctx.fillText(seperator, winsXPos + 84, yPos + teamLogoHeight / 4);
            // losses
            ctx.fillText(losses, winsXPos + 84 + 56, yPos + teamLogoHeight / 4);
            // 2nd seperator
            ctx.fillText(seperator, winsXPos + 84 + 56 + 56 + 28, yPos + teamLogoHeight / 4);
            // ties
            ctx.fillText(ties, winsXPos + 84 + 56 + 56 + 28 + 56, yPos + teamLogoHeight / 4);
            // 3rd seperator
            ctx.fillText(seperator, winsXPos + 84 + 56 + 56 + 28 + 56 + 84, yPos + teamLogoHeight / 4);
            // otl
            ctx.fillText(otl, winsXPos + 84 + 56 + 56 + 28 + 56 + 84 + 56, yPos + teamLogoHeight / 4);
        }

        // 4️⃣ Draw points or pct
        const criteria = byPts ? String(standing.Pts) : String(standing.Pct);
        const criteriaType = byPts ? "Pts" : "Pct";
        const xPosOfCriteria = messageContent === 'BUBBLE' ? 570 : 970 
