...
export default async function displayStandings(seasonGamesChannelId, { client, standings, messageContent, coaches }) {
    ...
    switch(messageContent){
        ...
        case "BUBBLE":
            ...
            teamLogoHeight = 50;
            teamLogoWidth = 50;
            description = 'WNHL 95 bubble teams';
            break;
        default:
    ...
    const xMargin = 20;
    const seedX = xMargin;
    const logoX = seedX + 50;
    let statsX, pointsX;

    if (messageContent === "STANDINGS" || messageContent === "STANDINGS%") {
        statsX = logoX + teamLogoWidth + 40;
        pointsX = statsX + 250;
    } else { // BUBBLE
        statsX = logoX + teamLogoWidth + 20;
        pointsX = statsX + 120;
    }
    ...
    for (const standing of sortedStandings) {
        ...
        ctx.fillText(`${wins}-${losses}-${ties}-${otl}`, statsX, yPos + teamLogoHeight / 4);

        // 4️⃣ Draw points or pct
        const criteria = byPts ? String(standing.Pts) : String(standing.Pct);
        const criteriaType = byPts ? "Pts" : "Pct";
        ctx.fillText(`${criteria} ${criteriaType}`, pointsX, yPos + teamLogoHeight / 4);
