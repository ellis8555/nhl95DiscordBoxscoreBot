...
async function displayRemainingOpponents(seasonGamesChannelId, {server, client, teamAbbreviation, teamCodes, coaches, uniqueIdsFile}){
    ...
    if(teamCodes.length > 1){
        // TODO: custom team removal to be addressed in the future
        // remove sunnyvale from the list
        const sunnyvale = teamCodes.indexOf("SUN");
        if(sunnyvale !== -1){
            teamCodes.splice(sunnyvale, 1)
        }
        // remove gophers from the list
        const gophers = teamCodes.indexOf("MGG");
        if(gophers !== -1){
            teamCodes.splice(gophers, 1)
        }
        // remove MattyA from the list
        const kaos = teamCodes.indexOf("KVK");
        if(kaos !== -1){
            teamCodes.splice(kaos, 1)
        }
