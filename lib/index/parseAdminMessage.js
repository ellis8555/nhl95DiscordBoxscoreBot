...
async function parseAdminMessage(adminsListeningChannelId, {server, adminMessage, client, csvFile, sheets}){
                ...
                switch(adminMessage[1]){
                    case "SETTINGS":
                        ...
                        const settings = {
                            ...
                            "Season games channel": serverConsts["seasonGamesChannel"],
                            ...
                            "Is playoffs": serverConsts["isPlayoffs"],
                            ...
                            "Update json standings on BSB server": serverConsts["updateStandings"],
                        ...
                        if(adminMessage[2] === "CSV"){
                            ...
                            if(adminMessage[3]){
                                ...
                                if(!isValidLeagueName){
                                    writeToFile = false
                                    response = `The league name should be one lowercase letter`
                                    break;
                                }
                            // season number entry
                            if(adminMessage[4]){
                            ...
                            if(!url || !fileName){
                                ...
                                const csvFilePath = path.join(__dirname, "..", "game-state-parsing", "read-csv", "csv", leagueName, seasonNumber)                            
    ...
    // Q league server
    if(server === q_server){
        const filePath = path.join(__dirname, "..", "..", "public", "json", "q_bot_constants.json")
        const readFile = fs.readFileSync(filePath, "utf-8")
        const serverConsts = JSON.parse(readFile);
        switch(adminMessage[0]){
            case "PAUSE":
