...
async function parseAdminMessage({server, adminMessage, client, adminBoxscoreChannelId, csvFile}){
            ...
            case "GET":
                switch(adminMessage[1]){
                    case "SETTINGS":
                        writeToFile = false;
                        const settings = {
                            listeningChannel: serverConsts["listeningChannel"],
                            outputChannel: serverConsts["outputChannel"],
                    ...
                    case "KEYWORDS":
                        ...
                        serverConsts.adminKeywords.splice(2,2)
                        serverConsts.adminKeywords[0] = "PAUSE - pauses BSB. Game state uploads are ignored. Admin commands still work."
                        serverConsts.adminKeywords[1] = "RESUME - resumes BSB. Game state uploads will be processed"
            ...
            case "SET":
                switch(adminMessage[1]){
                    ...
                    case "BOXSCORE":
                        ...
                        writeToFile = true
                        response = `Creating boxscore image has been ${adminMessage[2] === "TRUE"? "enabled":"disabled"}.`
                    break;
                    case "INPUT":
                        if(adminMessage[2] === "CHANNEL" && adminMessage[3]){
                            const searchForChannel = client.channels.cache.find(c => c.name === adminMessage[3]);
                            if(searchForChannel){
                                serverConsts.listeningChannel = adminMessage[3];
                                writeToFile = true;
                    ...
                    case "OUTPUT":
                        if(adminMessage[2] === "CHANNEL" && adminMessage[3]){
                            const searchForChannel = client.channels.cache.find(c => c.name === adminMessage[3]);
                            if(searchForChannel){
                                serverConsts.outputChannel = adminMessage[3];
                                writeToFile = true;
                                response = `Listening channel has been edited to ${adminMessage[3]}`
                            } else {
                                writeToFile = false;
                            ...
                            writeToFile = false
                            response = `A name for new output channel needs to be provided.`
                        }
                    break;
                    case "SAVE":
