...
async function showRemainingGames(channelId, {client, coachId, leagueName, messageContent}){
    ...
    try {
        ...
        for (let i = gamesPlayedArray.length - 1; i >= 0; i--) {
            const game = gamesPlayedArray[i];
            ...
            if (playersToIgnore.includes(team1) || playersToIgnore.includes(team2)) {
                gamesPlayedArray.splice(i, 1); // Remove the game if it contains an ignored team
            }
        }
        // count used to determine when to begin gathering remaining games data and displaying to user
        const gamesThatWillNotBePlayed = playersToIgnore.length * (teamsList.length - 1)
        const updatedTotalGamesToBePlayed = totalLeagueGamesPlayed - gamesThatWillNotBePlayed
        const gamesRemainingToBePlayed = updatedTotalGamesToBePlayed - gamesPlayedArray.length
        
        if(gamesRemainingToBePlayed > 20){
            await channel.send(`${gamesRemainingToBePlayed} left to be played`)
        }
        if(gamesRemainingToBePlayed < 21 && gamesRemainingToBePlayed !== 0){
            ...
            if(parsedLeagueConsts.remainingGames){
                ...
                gamesPlayedArray.forEach(match => {
