...
async function showRemainingGames(channelId, {client, coachId, leagueName, messageContent}){
    ...
    try {
        if(leagueName === "W"){
            ...
            leagueConstsFileName = "bot_constants.json"
        ...
        // display each remaining matchup for when games remaining <= set number
            if(gamesRemainingToBePlayed < 51 && gamesRemainingToBePlayed !== 0){ 
                ...
                if(leagueGamesRemaining.length > 0){
                    ...
                    try {                        
                        // write the remaining matchups to the constants file
                        const filePath = join(process.cwd(), "public", "json", leagueConstsFileName)
