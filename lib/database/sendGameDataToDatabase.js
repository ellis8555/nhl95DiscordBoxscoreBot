async function sendGameDataToDatabase({league, isPlayoffs, coaches, romData, seeds}){
    ...
    try {
        ...
        // teamStats
        const { homeTeamGameStats, awayTeamGameStats, otherGameStats, allGoalsScored, homeTeamPlayerStats, awayTeamPlayerStats, homeTeamGoalieStats, awayTeamGoalieStats } = romData
        ...
        // players stats
        // extract home player stats and push to skaters array
        const filterForHomeTeamPlayerObjects = homeTeamPlayerStats.filter(player => player.Name !== "-")
        filterForHomeTeamPlayerObjects.forEach(player => {
            const playerObj = {}
            playerObj['team_code'] = homeTeam
            playerObj['player_name'] = player.Name
            playerObj['goals'] = player.G
            playerObj['assists'] = player.A
            playerObj['points'] = player.PTS
            playerObj['shots'] = player.SOG
            playerObj['chk'] = player.CHK
            playerObj['pim'] = player.PIM
            playerObj['toi'] = player.TOI
            skaters.push(playerObj)
        })

        // extract away player stats and push to skaters array
        const filterForAwayTeamPlayerObjects = awayTeamPlayerStats.filter(player => player.Name !== "-")
        filterForAwayTeamPlayerObjects.forEach(player => {
            const playerObj = {}
            playerObj['team_code'] = awayTeam
            playerObj['player_name'] = player.Name
            playerObj['goals'] = player.G
            playerObj['assists'] = player.A
            playerObj['points'] = player.PTS
        ...
        // extract home goalie stats and push to goalies array
        const filterForHomeTeamGoalieObjects = homeTeamGoalieStats.filter(goalie => goalie.Name !== "-")
        filterForHomeTeamGoalieObjects.forEach(goalie => {
            const goalieObj = {}
            goalieObj['team_code'] = homeTeam
            goalieObj['player_name'] = goalie.Name
            goalieObj['goals'] = goalie.G
            goalieObj['assists'] = goalie.A
            goalieObj['points'] = goalie.PTS
            goalieObj['shots'] = 0
            goalieObj['goals_against'] = goalie.GA
            goalieObj['saves'] = goalie.SV
            goalieObj['shots_against'] = goalie.SH
            goalieObj['shutouts'] = goalie.SO
            goalieObj['toi'] = goalie.TOI
            goalies.push(goalieObj)
        })
        // extract away goalie stats and push to goalies array
        const filterForAwayTeamGoalieObjects = awayTeamGoalieStats.filter(goalie => goalie.Name !== "-")
        filterForAwayTeamGoalieObjects.forEach(goalie => {
            const goalieObj = {}
            goalieObj['team_code'] = awayTeam
            goalieObj['player_name'] = goalie.Name
            goalieObj['goals'] = goalie.G
            goalieObj['assists'] = goalie.A
            goalieObj['points'] = goalie.PTS
        ...
        if (!result.ok) {
        throw new Error("DB: Game record failed to insert or update");
        }

        // team stats
        if (result.teamStats && !result.teamStats.ok) {
        throw new Error("DB: Team stats failed to insert");
        }

        // scoring plays
        if (result.scoring && !result.scoring.ok) {
        throw new Error("DB: Scoring plays failed to insert");
        }       
