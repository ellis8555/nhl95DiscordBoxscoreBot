...
async function generateCanvasImage({
    ...
    const bannerScoreFont = "Arial"; // score fonts
    ...
    const headersFont = "Arial"; // headers
...
awayTeamPlayerStats.sort(sortPlayerStats);
    ...
    const wLogo = await loadImage( // add league logo
      path.join(__dirname, "public", "images", "league", `w.png`)
    );
  
    const wLogoStartY = awayTeamLogoX;
    const wLogoHeight = 150;
    const wLogoWidth = 150;
    // Draw the logos on the canvas
    ctx.drawImage(wLogo, xCenter-wLogoWidth/2, wLogoStartY, wLogoWidth, wLogoHeight); // keep in line with away team banner
    ...
    const scoringGridStartY = wLogoStartY + wLogoHeight + 50;
    ...
    const awayTeamsGameStatsArray = [
      $["AwaySHOTS"].toString(),
      $["Away1ST GOALS"].toString(),
      $["Away2ND GOALS"].toString(),
      $["Away3RD GOALS"].toString(),
      $["AwayOT GOALS"].toString(),
...
const homeTeamsGameStatsArray = [
  $["HomeSHOTS"].toString(),
  $["Home1ST GOALS"].toString(),
  $["Home2ND GOALS"].toString(),
  $["Home3RD GOALS"].toString(),
  $["HomeOT GOALS"].toString(),
