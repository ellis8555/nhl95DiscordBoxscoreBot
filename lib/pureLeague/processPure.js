...
const {
    ...
    manager_col_start,
    streak_col,
    managersNameRowStart,
    managersRowPostValue
} = appendGoogleSheets.standings
        ...
        try {            
            if (homeTeamRowNumberFound) {
                ...
                if (requests.length > 0) {
                    ...
                    range = `Standings!${manager_col_start}:${manager_col}`
                    ...
                    const findPlayersEnd = manager_rows.indexOf(managersRowPostValue)
                    // remove entries that are not players
                    manager_rows.splice(findPlayersEnd)

                    // get recently updated coach objects which include streak
                    const filepath = path.join(process.cwd(), "public", "json", "pure_bot_constants.json");
                    ...
                    // loop through each manager in the standings sheet and associate each managers streak
                    ...
                    manager_rows.forEach((manager, index) => {
                        ...
                        coachDetails.sheetsStandingsRowNumber = index + +managersNameRowStart
                    ...
                    managerStreakObjects.forEach(manager => {
                        updateStreakRequests.push({
                            range: `Standings!${streak_col}${manager.sheetsStandingsRowNumber}`,
                            values: [
