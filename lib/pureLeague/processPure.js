...
async function processPure({sheets, message}){
    ...
    const scoreParts = getScore.split(" ").filter(part => part !== "" && part !== "-"); // split up the entered score remove space
    ...
    if(scoreParts[0] === "EDIT"){
        ...
        if(authorizedEditors.includes(getAuthorId)){
            ...
            throw new Error("Permission required to make edits.") // unauthorized
        }
    }
