...
const readPlayoffsFile = fs.readFileSync(playoffsFilePath, "utf-8")
...
// parse W league playoffs
const watch_playoffs_consts = JSON.parse(readPlayoffsFile)
// parse Q league
...
const w_playoffs_update_emitter=  new EventEmitter()
...
// watch W league playoffs file
fs.watchFile(playoffsFilePath, (curr, prev) => {
    if (curr.mtime !== prev.mtime) {
        const updatedConsts = JSON.parse(fs.readFileSync(playoffsFilePath, "utf-8"));
        Object.assign(watch_playoffs_consts, updatedConsts);
        w_playoffs_update_emitter.emit("w_playoffs_update_emitter", watch_playoffs_consts);
    }
});
