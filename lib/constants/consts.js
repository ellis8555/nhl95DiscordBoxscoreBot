...
const q_bot_consts = {
    token : `${process.env.token}`, // token or dev-token
    uniqueIdsFileName: `${process.env.qUniqueIdsFileName}`, // the test for duplicates/invalid states
    server: `${process.env.qServer}`,
    ...q_watched_bot_consts,
};
console.log(q_bot_consts)
...
fs.watchFile(qFilePath, async (curr, prev) => {
    if(curr.mtime !== prev.mtime){
        ...
        q_bot_consts_update_emitter.emit("bot_consts_update_emitter", q_bot_consts)
    }
})

export { bot_consts, bot_consts_update_emitter, q_bot_consts_update_emitter }